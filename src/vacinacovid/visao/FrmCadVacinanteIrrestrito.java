 ...
 * @author maxwell
 */
public class FrmCadVacinanteIrrestrito extends JDialog {
    ...
    private JComboBox cbLaboratorio, cbStatus, cbEsf, cbAcs;
    private JButton btSalvar, btCancelar;
    private ImageIcon salvar, cancelar, covid, vacina;
    private JPanel pnFundo;

    public FrmCadVacinanteIrrestrito(JFrame parent, boolean modal, String titulo,
    ...
    private void inicializarComponentesDaTela() {
        ...
        try {
            ...
            Logger.getLogger(FrmCadVacinanteIrrestrito.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void salvar() {
        ...
        if(cbEsf.getSelectedIndex() == 0){
            v.setAgente(new AcsBean(0, "", new UbsBean(0, "")));
        }else{
            v.setAgente(cbAcs.getSelectedItem().toString());
        }

        VacinanteDAO dao = new VacinanteDAO();
        if (dao.insert(v)) {
            JOptionPane.showMessageDialog(this, "Cadastro realizado com sucesso!", "Cadastro confirmado", JOptionPane.INFORMATION_MESSAGE, vacina);
            if(Controle.principalGestor.cbEsf.getSelectedItem().equals("Todos")){
                Controle.principalGestor.pesquisar(Controle.principalGestor.modelo, "Todos");
            }else{
                Controle.principalGestor.cbEsf.setSelectedItem(cbEsf.getSelectedItem().toString());
                Controle.principalGestor.pesquisar(Controle.principalGestor.modelo, Controle.principalGestor.cbEsf.getSelectedItem().toString());
            }
    ...
    private void carregaCbAcs() {
        ...
        for (AcsBean acs : dao.select("ESF " + cbEsf.getSelectedItem().toString().toUpperCase())) {
            cbAcs.addItem(Utilidades.iniciaisMaiuscula(acs.getNome()));
        }
